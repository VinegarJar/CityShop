...
#import "KnowCommitteeView.h"
@interface OtherwiseForemostDoseView ()
{
    int _random1;
    int _random2;
    int _random3;
...
@end
@implementation OtherwiseForemostDoseView
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.userInteractionEnabled = NO;
        self.colViews = [NSMutableArray array];
        for (int i = 0; i < 3; i++) {
            KnowCommitteeView *colView = [KnowCommitteeView viewFromNib];
...
- (void)directTowardBadminton {
    ...
    for (int i = 0; i < 3; i++) {
        switch (i) {
            case 0:
                [(KnowCommitteeView *)self.colViews[i] suggestConsequentlyStressfulAeroplane:_random1];
                break;
            case 1:
                [(KnowCommitteeView *)self.colViews[i] suggestConsequentlyStressfulAeroplane:_random2];
                break;
            case 2:
                [(KnowCommitteeView *)self.colViews[i] suggestConsequentlyStressfulAeroplane:_random3];
                break;
            default:
                break;
        }
...
- (void)sinInsideFerry:(NSInteger)index {
    if (index == 0) {
        ...
        [UIView animateWithDuration:0.15 animations:^{
            ...
            [[YaBoOrgyTool sharedSoundToolManager] patWorthyLiberty:YaSoundArrivalName];
            [self examineState];
        }];
    }
    if (index == 1) {
