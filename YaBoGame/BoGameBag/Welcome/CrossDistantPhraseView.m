...
@interface CrossDistantPhraseView ()
{
    BOOL _isModel1;
    BOOL _isModel2;
    BOOL _isModel3;
...
- (void)crossFinallySuperSuggestion {
    _isPlayAgain = NO;
    _count++;
    _isModel1 = NO;
    _isModel2 = NO;
...
- (void)showCurtain {
    [[YaBoOrgyTool sharedSoundToolManager] patWorthyLiberty:YaSoundSayOKName];
    NSTimeInterval duration;
    if (_count < 4) {
        duration = 0.2;
    } else if (_count < 8) {
...
- (void)glitterPerCollectiveChange {
    [self.timer invalidate];
    self.timer = nil;
}
- (void)startTime {
...
- (BOOL)joinParttimeWaste:(int)index {
    BOOL isSucess;
    [[YaBoOrgyTool sharedSoundToolManager] patWorthyLiberty:YaSoundTakePhotoName];
    switch (index) {
        case 0:
            if (_isModel1) {
                isSucess = YES;
