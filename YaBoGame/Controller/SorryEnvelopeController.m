...
#import "SuccessfullyMobileHometownView.h"
#import "WashOntoGreenManager.h"
#import "ShouldRipeGraceController.h"
@interface SorryEnvelopeController ()
@property (weak, nonatomic) IBOutlet SuccessfullyMobileHometownView *scoreView;
@property (weak, nonatomic) IBOutlet HushByPaceView *animationView;
...
@end
@implementation SorryEnvelopeController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)startTitleAnimation {
    ...
    for (int i = 0; i < self.labels.count; i++) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((i + 1) * 0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [[YaBoOrgyTool sharedSoundToolManager] patWorthyLiberty:YaSoundPrepaerTitle(i+1)];
        });
    }
...
- (void)showPlayView {
    ...
    [self.scoreView negotiatePolitelyMarvelousNight:^{
