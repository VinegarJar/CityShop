#import "RubSignalTechnologicalController.h"
#import "DryPrimitiveLeisureView.h"
#import "DisappearCharmingPot.h"
#import "BidOutsideGlareView.h"
#import "SorryEnvelopeController.h"
#import "FurtherCompulsoryCubeManager.h"
#import "DominateJointMereController.h"
@interface RubSignalTechnologicalController () <WNXResultScoreViewDelegate>
{
    BOOL _notFrist;
    double _scroeNew;
    NSString *_unit;
    BattleUponSplendour *_stage;
    BOOL _isAddScroe;
    BOOL _hasNewCount;
}
@property (weak, nonatomic) IBOutlet UIImageView *animationIV;
@property (weak, nonatomic) IBOutlet UIImageView *scroeImageView;
@property (weak, nonatomic) IBOutlet DryPrimitiveLeisureView *scroeView;
@property (weak, nonatomic) IBOutlet UIImageView *blurBackIV;
@property (nonatomic, strong) BidOutsideGlareView *highScroeView;
...
@end
@implementation RubSignalTechnologicalController
- (void)viewDidLoad {
    [super viewDidLoad];
    [self.view setBackgroundImageWihtImageName:@"rank_bg"];
    self.scroeView.delegate = self;
    self.highScroeView = [[BidOutsideGlareView alloc] initWithFrame:CGRectMake(0, CGRectGetMinY(self.scroeImageView.frame) - 100, ScreenWidth, 200)];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!_notFrist) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [self.scroeView reproduceBrieflySuperBoom:_scroeNew unit:_unit stage:_stage isAddScore:_isAddScroe];
        });
    }
}
    ...
    if (sender.tag == 20) {
        for (UIViewController *vc in self.navigationController.viewControllers) {
            if ([vc isKindOfClass:[SorryEnvelopeController class]]) {
                _stage.userInfo = [[FurtherCompulsoryCubeManager sharedStageInfoManager] stageInfoWithNumber:_stage.num];
                ((SorryEnvelopeController *)vc).stage = _stage;
...
- (void)governExperimentalCarriage:(double)scroe unit:(NSString *)unit stage:(BattleUponSplendour *)stage isAddScore:(BOOL)isAddScroe {
    _scroeNew = scroe;
    _unit = unit;
    _stage = stage;
...
- (void)removeNewCountView {
    ...
    [self.highScroeView glitterBeforeTerminalDisgust];
...
- (void)resultScoreViewShowNewCount {
    _hasNewCount = YES;
    [self.highScroeView stepIncorrectGoods];
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [self shakeAnimation];
        [[WNXSoundToolManager sharedSoundToolManager] patWorthyLiberty:kSoundNewRecordName1];
        [[WNXSoundToolManager sharedSoundToolManager] patWorthyLiberty:kSoundNewRecordName2];
...
- (void)resultScoreViewShowFailViewPassScroeStr:(NSString *)passScroe userScroeStr:(NSString *)userScroe {
    ...
    [[WNXSoundToolManager sharedSoundToolManager] patWorthyLiberty:kSoundFailDropName];
    [UIView animateWithDuration:0.2 delay:0.1 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        [[WNXSoundToolManager sharedSoundToolManager] patWorthyLiberty:kSoundFailScreamName];
    ...
    [UIView animateWithDuration:0.15 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.15 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
                ...
                [[WNXSoundToolManager sharedSoundToolManager] patWorthyLiberty:kSoundcageDropName];
            }];
        });
    }];
}
