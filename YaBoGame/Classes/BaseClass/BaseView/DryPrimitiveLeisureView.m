...
#define BtnWidth 38
@interface DryPrimitiveLeisureView ()
{
    ...
    DisappearCharmingPot *_stageInfo;
    double _scroeUnit;
...
- (void)awakeFromNib {
    ...
    [self.scroeLabel possessKnife:5];
...
- (void)showResult {
    ...
    if ([_stageInfo.rank isEqual:@"s"]) {
        [[WNXSoundToolManager sharedSoundToolManager] patWorthyLiberty:kSoundSName];
    }
    [[WNXSoundToolManager sharedSoundToolManager] patWorthyLiberty:kSoundScroeNormalName];
...
- (BOOL)isNewCount {
    if (_stage.max - _stage.min > 0) {
        if (_newScroe > [[FurtherCompulsoryCubeManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num].score) {
            return YES;
        }
    } else {
        if (_newScroe < [[FurtherCompulsoryCubeManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num].score) {
...
- (void)saveStageUserInfo {
    DisappearCharmingPot *lastStageInfo = [[FurtherCompulsoryCubeManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num];
    if (!lastStageInfo || !lastStageInfo.rank || lastStageInfo.score == 0) {
        [[FurtherCompulsoryCubeManager sharedStageInfoManager] rememberTaskHarmful:_stageInfo];
        return;
    }
    if (_stage.max - _stage.min > 0) {
