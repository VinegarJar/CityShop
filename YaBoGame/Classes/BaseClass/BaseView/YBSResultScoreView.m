...
@interface YBSResultScoreView ()
{
    ...
    double _newScroe;
    YBSStage *_stage;
    YBSStageInfo *_stageInfo;
    double _scroeUnit;
    double _currentScroe;
    BOOL _isAddScroe;
}
@end
@implementation YBSResultScoreView
- (void)awakeFromNib {
    _stageInfo = [[YBSStageInfo alloc] init];
    for (int i = 0; i < 7; i++) {
...
- (BOOL)isNewCount {
    if (_stage.max - _stage.min > 0) {
        if (_newScroe > [[YBSStageInfoManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num].score) {
            return YES;
        }
    } else {
        if (_newScroe < [[YBSStageInfoManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num].score) {
...
- (void)saveStageUserInfo {
    YBSStageInfo *lastStageInfo = [[YBSStageInfoManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num];
    if (!lastStageInfo || !lastStageInfo.rank || lastStageInfo.score == 0) {
        [[YBSStageInfoManager sharedStageInfoManager] saveStageInfo:_stageInfo];
        return;
    }
    if (_stage.max - _stage.min > 0) {
