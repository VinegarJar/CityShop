...
@interface LastMedicalSuspensionView ()
...
@end
@implementation LastMedicalSuspensionView
...
- (void)consistOnYearlyBlowhole:(NSInteger)index type:(WNXStage13BottomType)type {
...
- (void)coolPanda:(NSInteger)index {
    if (index == 0) {
        self.manIV.transform = CGAffineTransformMakeScale(0, 0);
...
- (void)interiorMathematician:(NSInteger)index {
    if (index == 0) {
        self.manIV.hidden = YES;
    } else if (index == 1) {
        self.childIV.hidden = YES;
...
- (void)exclaimDespiteSuitableDeal {
    [self consistOnYearlyBlowhole:0 type:WNXStage13BottomTypePeople];
    [self consistOnYearlyBlowhole:1 type:WNXStage13BottomTypePeople];
    [self consistOnYearlyBlowhole:2 type:WNXStage13BottomTypePeople];
    self.manIV.hidden = YES;
    self.childIV.hidden = YES;
    self.oldIV.hidden = YES;
}
