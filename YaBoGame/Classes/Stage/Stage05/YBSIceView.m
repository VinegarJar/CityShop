...
@end
@implementation YBSIceView
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.userInteractionEnabled = NO;
        self.colViews = [NSMutableArray array];
        for (int i = 0; i < 3; i++) {
            YBSColumnIceView *colView = [YBSColumnIceView viewFromNib];
...
- (void)showDottedLineView {
    ...
    for (int i = 0; i < 3; i++) {
        switch (i) {
            case 0:
                [(YBSColumnIceView *)self.colViews[i] showColumnDottedLineWithNum:_random1];
                break;
            case 1:
                [(YBSColumnIceView *)self.colViews[i] showColumnDottedLineWithNum:_random2];
                break;
            case 2:
                [(YBSColumnIceView *)self.colViews[i] showColumnDottedLineWithNum:_random3];
                break;
            default:
                break;
        }
