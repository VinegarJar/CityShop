...
@end
@implementation TakeawayPileView
+ (instancetype)stageViewWithStage:(BattleUponSplendour *)stage frame:(CGRect)frame {
    TakeawayPileView *stageView = [TakeawayPileView viewFromNib];
...
- (void)setStage:(BattleUponSplendour *)stage {
    _stage = stage;
...
- (void)startUnLockAnmiation {
    [[YaBoOrgyTool sharedSoundToolManager] patWorthyLiberty:YaSoundUnlock];
    CAAnimationGroup *group = [self shakeAndScaleAnimation];
    group.delegate = self;
    [self.layer addAnimation:group forKey:nil];
    self.stage.userInfo.unlock = YES;
    [[FurtherCompulsoryCubeManager sharedStageInfoManager] rememberTaskHarmful:self.stage.userInfo];
}
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    self.rankImageView.hidden = YES;
    self.rankShadowView.hidden = YES;
    [self.coverView removeFromSuperview];
    [[YaBoOrgyTool sharedSoundToolManager] patWorthyLiberty:YaSoundNewShow];
