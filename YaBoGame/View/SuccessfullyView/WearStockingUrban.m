...
@interface WearStockingUrban ()<UITextViewDelegate>
@property(nonatomic,strong)BelieveOutputEachView *sportsAlert;
@property(nonatomic,strong)UIView *sportsBlackView;
@end
@implementation WearStockingUrban
+(instancetype)initYBSportsAlertWithFrame:(CGRect)frame delegate:(id<YBSDCAlertDelegate>)delegate{
    WearStockingUrban *alertView = [[self alloc] initWithFrame:frame];
...
-(BelieveOutputEachView*)sportsAlert{
    if (!_sportsAlert) {
        _sportsAlert = [BelieveOutputEachView loadNibNamedPrivacyView];
...
-(void)clicked:(UIButton *)sender{
    ...
    [self demonstratePremierArtistic];
}
